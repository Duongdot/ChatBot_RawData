(function (factory) {
    ...
    return function koreBotChat() {
        ...
        var returnFun =(function ($,KRPerfectScrollbar) {
            ...
            let dataSubject = []
            let itemSubjects =  []
            let childrenItemSubject = []
            let valueSelected = ""
            var isSelectionSubject = false;
        ...
        function updateOnlineStatus() {
            ...
            chatWindow.prototype.getItemSubject = function (subject) {
                valueSelected = subject
                if(subject) {
                    const itemSubjects = dataSubject.find(item => {
                        return item.title === subject
                    })
                    $("#button_programing_item").empty()
                    return itemSubjects.children.map((item) => {
                        $("#button_programing_item").append(`<button class="button-items">${item.title}</button>`);
                        return ;
                    })
                }
            }
            ...
            chatWindow.prototype.bindEvents = function () {
                ...
                _chatContainer.off('change', '#programing').on('change', '#programing', function (event) {
                    isSelectionSubject = true
                    const valueSelected = event.target.value
                    if(valueSelected) {
                        me.getItemSubject(valueSelected)
                    }
                })
                ...
                _chatContainer.off('click', '.button-items').on('click', '.button-items', function (event) {
                    $("#button_programing_item").remove()
                    const value = event.target.innerText
                    const message = `Subject: ${valueSelected}, Item: ${value}`;
                    $(".chatInputBox").text(message);
                ...
                if (messageHtml === '' && msgData && msgData.message && msgData.message[0]) {
                    ...
                    if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "button") {
                        ...
                        msgData.message[0].component.payload.template_type ==
                          "select_programing_language"
                      ) {
                        messageHtml = $(me.getChatTemplate("dropDownTemplate")).tmpl({
                          msgData: msgData,
                          helpers: helpers,
                          extension: extension,
                        });
     ...
     var iframe = '<script id="chat_message_tmpl" type="text/x-jquery-tmpl"> \
            ...
            else if (tempType === "dropDownTemplate") {
            ...
            chatWindow.prototype.startDropDown = async function() {
                const initValue = $("#programing option:selected").text()
                console.log(initValue)
                await $.getJSON("../data/data.json", (data) => {
                    dataSubject = data
                })
